
#include <stdio.h>
#include <stdlib.h>
#include "include/Buddy.h"
#include "include/Header.h"

#define malloc mymalloc
#define free myfree

#define MAX_REQUEST 4096
#define MAX_ITEMS 100

#define SILENT 0
#define TERSE  1
#define VERBOSE  2
#define INTERACTIVE 3

int verbosity = TERSE;


void simpleFreeTest()
{
	char *x;

	// malloc one byte to make buddy system
	// split all the way down
	x = (char *) malloc(sizeof(char));
	if (verbosity > 1) printBuddyLists();
	// free should make all the blocks merge back into one block
	free(x);
	if (verbosity > 1) printBuddyLists();

}

void simpleAllocationTest()
{
	void *ptr;
	size_t count;

	//malloc larger and larger blocks until memory is exhausted
	count = 1;
	for (;;) {
		ptr = malloc(count);
		if (ptr == NULL) {
			perror("TestBuddy");
			exit(1);
		}
		if (verbosity > 1) printBuddyLists();
		if (verbosity > 0)
			printf("malloc'd a block of size %u\n", count);
		count = count * 10;
	}
}

struct element {
	char *ptr;
	size_t size;
};

int main(int argc, char *argv[])
{
	int i;
	char ch;
	int count;
	unsigned long int seed;
	size_t size;
	struct element x[MAX_ITEMS];
	int loc;

	if (argc < 2) {
		fprintf(stderr, "Usage: %s <num of tests> [random seed] [silent|terse|verbose|interactive]\n", argv[0]);
		exit(1);
	}
	count = atol(argv[1]);
	if (argc == 3) {
		seed = atol(argv[2]);
		srandom(seed);
	}
	if (argc == 4) {
		if (argv[3][0] == 's') {
			verbosity = SILENT;
		} else if (argv[3][0] == 't') {
			verbosity = TERSE;
		} else if (argv[3][0] == 'v') {
			verbosity = VERBOSE;
		} else if (argv[3][0] == 'i') {
			verbosity = INTERACTIVE;
			setvbuf(stdin, NULL, _IONBF, 0);
		}
	}

	initBuddySystem(0);
	if (verbosity > 0) printBuddyLists();

	simpleFreeTest();




	for (i =0; i < MAX_ITEMS; i++) {
		x[i].ptr = NULL;
		x[i].size = 0;
	}


	for (i=0; i < count; i++) {
		loc = random() % MAX_ITEMS; // where to put in our table
		if (x[loc].ptr) {
			if (verbosity > 1) printBuddyLists();
			free(x[loc].ptr);
			if (verbosity > 0)
				printf("freed address %p of size %d  in x[%d]\n", x[loc].ptr, x[loc].size, loc);
			x[loc].ptr = NULL;
			x[loc].size = 0;
		} else {
			size = random() % MAX_REQUEST + 1; // how big a request
			if (verbosity > 1) printBuddyLists();
			x[loc].ptr = (char *) malloc(size*sizeof(char));
			if (x[loc].ptr == NULL) {
				perror("TestBuddy:");
				exit(1);
			}
			x[loc].size = size*sizeof(char);
			memset(x[loc].ptr, '1', x[loc].size);
			if (verbosity > 1) printBuddyLists();
			if (verbosity > 0)
				printf("malloced %u bytes and stored address %p at x[%d]\n", size*4, x[loc].ptr, loc);
		}

		if (verbosity == INTERACTIVE) {
			ch = getchar();
			system("clear");
			if (ch == 'q')
				exit(0);
		}

	}
#ifdef STATS
	printBuddySystemStats();
#endif


	simpleAllocationTest();

	exit(0);
}

